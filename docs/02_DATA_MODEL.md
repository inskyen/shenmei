...
posts = 使用者對某支影片的一次策展推薦，負責信息流傳播與個人審美表達
```

同一支影片可以被不同使用者用不同推薦理由多次策展。
...
- `posts`：公開 `published` 可讀，登入使用者可建立，作者可更新/刪除自己的。
- `modules`：公開可讀，管理員可建立/更新。
- `comments`：公開可讀，登入使用者可建立，作者可刪除自己的；讀寫時需檢查 `target_type` 對應目標是否可見。
