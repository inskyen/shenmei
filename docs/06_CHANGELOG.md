...
- 首頁拆分為「追蹤／推薦／最新／頻道」，預設進入推薦流；最新流繼續按採樣時間排序。
- 新增獨立推薦 API，按頻道偏好、追蹤、新鮮度、互動與隨機探索形成候選分數。
- 推薦批次加入同影片去重、作者與頻道多樣性整理，以及近期曝光排除。
- 推薦卡片顯示簡短原因，讓使用者知道內容為何出現。
- 頻道頁新增「不推薦／少一點／正常／多一點」四檔主動調節。
- 新增 `user_channel_preferences` 與 `feed_impressions`；不記錄停留時長。
- 訪客使用公共推薦配方，登入使用者才保存個人頻道偏好與曝光去重。

### 驗證

- `npm run lint`：0 errors；保留既有 `<img>` 最佳化 warnings。
- `npm run build`：通過。
- 本地推薦 API 已返回真實 Supabase 採樣、推薦原因與頻道資料。
- 首頁四欄與頻道偏好控件已完成瀏覽器檢查。
...
- 全面同步 README、產品簡報、路由、API、技術指南、部署手冊、路線圖與 AI 交接文檔，將專案階段更新為「核心 MVP 完成／種子內測準備」。
- 明確公開測試前的限流、檢舉封禁、接口安全、協議、監控、備份與測試阻斷項。
- 統一審美號規則：註冊時生成唯一數字，前端與資料庫禁止使用者自行修改。
- 採樣與留言加入軟刪除能力，作者可刪除自己的內容，超管可處理任何內容。
- 刪除採樣後保留影片本體，並同步清理首頁、追蹤、頻道、個人頁與詳情快取。
- 已刪除的主留言以占位保留回覆脈絡；無回覆的已刪除留言不再展示。
- 留言計數觸發器支援 `published → deleted` 狀態變化，避免軟刪除後數字殘留。
- 詳情頁加入刪除入口、二次確認與完成回饋。
...
- 首頁最新流改用三張採樣卡骨架載入，取代純文字等待提示。
- 離開首頁時記住捲動位置，從詳情返回後在首頁資料完成渲染時還原。
- 詳情頁返回鈕改為優先返回上一層；直接開啟連結時才回首頁。
- 將 `/p/[id]` 的獨立資料請求改為並行，並修正訪客狀態下靜默身份檢查會中斷公開頁載入的問題。
- 設定頁新增審美標籤與私訊權限編輯，直接保存至既有 `profiles` 欄位。
- 使用者頁新增審美標籤名片、本人發佈入口與骨架載入；未實作的關注改為明確的準備中狀態。
- 設定與發佈頁加入滑入與骨架過渡；發佈成功時先顯示「已放進最新大廳」回饋，再返回首頁。
- `/p/[id]` 留言區正式接入 `comments`：公開讀取、登入後發佈、即時顯示與 `comment_count` 自動計數均已驗證。
...
- `username` 註冊時自動生成數字；後續曾規劃允許修改，目前已由 2026-07-14 migration 改為鎖定。
- `modules.owner_id` 第一版保留但不提供頻道管理功能，可為空。
...
- 將 `/m` 從占位頁改為真實頻道列表頁。
- 頻道列表會讀取 active `modules`，展示名稱、slug、介紹、封面或主色塊。
- 將 `/m/[slug]` 從占位頁改為真實頻道詳情頁。
- 頻道詳情會按 slug 讀取 `modules`，並讀取 `post_modules -> posts -> videos` 展示頻道內採樣。
- 頻道詳情會批量讀取 `profiles`，展示發布者與推薦理由。
- 頻道內採樣可點進 `/p/[id]`，封面可點進 `/v/[id]`。
- 頻道空狀態、載入狀態與錯誤狀態已補齊。
...
- 使用者頁會讀取 `profiles`，展示 username、頭像、簡介占位與採樣統計。
