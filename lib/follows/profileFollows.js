...
export const FOLLOWING_FEED_PAGE_SIZE = 8;
...
export async function loadFollowingFeed({ offset = 0, limit = FOLLOWING_FEED_PAGE_SIZE } = {}) {
  const followedProfiles = await loadFollowedProfiles();
  if (followedProfiles.requiresLogin || followedProfiles.profiles.length === 0) {
    return { ...followedProfiles, posts: [], hasMore: false, nextOffset: 0 };
  }

  const profileIds = followedProfiles.profiles.map((profile) => profile.id);
  const profilesById = followedProfiles.profiles.reduce((result, profile) => {
    result[profile.id] = profile;
    return result;
  }, {});
