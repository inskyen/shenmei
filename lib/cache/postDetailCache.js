...
export function removeCachedPostDetail(postId) {
  postDetailCache.delete(postId);
}

export function adjustCachedPostCommentCount(postId, delta) {
  ...
  postDetailCache.set(postId, {
    ...
    post: {
      ...cached.post,
      comment_count: Math.max(0, (cached.post.comment_count || 0) + delta),
    },
  });
}
