    ...
    <div style={{ backgroundColor: '#F0F4F8', minHeight: '100vh', color: '#2A527A', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', paddingBottom: '100px', position: 'relative' }}>
      ...
      <header style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'baseline', gap: '20px' }}>
          <span style={{ color: '#87ACCA', fontSize: '18px', fontWeight: '500' }}>Following</span>
          <span style={{ color: '#2A527A', fontSize: '24px', fontWeight: 'bold' }}>Recent</span>
          <span style={{ color: '#87ACCA', fontSize: '18px', fontWeight: '500' }}>Party</span>
        </div>
      ...
      <main style={{ maxWidth: '600px', margin: '0 auto', paddingTop: '8px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', color: '#87ACCA', padding: '40px', fontSize: '14px' }}>流云正载着美好赶来...</div>
        ) : (
          <div>
            {videos.map((video) => (
              // 帖子卡片
              <article key={video.id || video.bvid} style={{ backgroundColor: '#FFFFFF', margin: '16px', borderRadius: '16px', padding: '16px', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid rgba(194, 214, 230, 0.3)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                    ...
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <span style={{ color: '#2A527A', fontWeight: '500', fontSize: '14px' }}>{video.added_by || 'Souler'}</span>
                        <span style={{ backgroundColor: '#6B99C3', color: '#FFFFFF', fontSize: '10px', padding: '2px 6px', borderRadius: '4px' }}>Hidden</span>
                      </div>
      ...
      {/* 悬浮发帖按钮 (融雪柔桃点缀) */}
      <div style={{ position: 'fixed', bottom: '96px', right: '16px', width: '48px', height: '48px', backgroundColor: '#FFFFFF', borderRadius: '50%', boxShadow: '0 4px 14px rgba(42, 82, 122, 0.15)', border: '1px solid #C2D6E6', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#2A527A', zIndex: 30, cursor: 'pointer' }}>
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, height: '80px', backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(194, 214, 230, 0.5)', display: 'flex', justifyContent: 'space-around', alignItems: 'center', padding: '8px 8px 16px', zIndex: 20 }}>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#87ACCA' }}>
          ...
          <span style={{ fontSize: '10px' }}>Planet</span>
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#2A527A' }}>
          <svg style={{ width: '24px', height: '24px', marginBottom: '4px' }} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
          <span style={{ fontSize: '10px', fontWeight: '500' }}>Explore</span>
        </div>
        {/* 核心加号按钮：矢车菊蓝悬浮 */}
        <div style={{ width: '48px', height: '48px', borderRadius: '50%', backgroundColor: '#6B99C3', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 10px rgba(107, 153, 195, 0.4)', transform: 'translateY(-16px)', border: '4px solid #FFFFFF', color: '#FFFFFF', cursor: 'pointer' }}>
      ...
      {detailPageVideo && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: '#F0F4F8', zIndex: 999, overflowY: 'auto' }}>
          <div style={{ display: 'flex', alignItems: 'center', padding: '16px', backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, borderBottom: '1px solid rgba(194, 214, 230, 0.5)' }}>
            ...
            <div style={{ flex: 1, textAlign: 'center', fontWeight: 'bold', color: '#2A527A', paddingRight: '40px' }}>动态详情</div>
          </div>
          <div style={{ padding: '20px', paddingBottom: '100px' }}>
            ...
            <div style={{ color: '#87ACCA', fontSize: '14px', marginTop: '10px' }}>UP主：<span style={{ color: '#6B99C3' }}>{detailPageVideo.up_name}</span></div>
